...
import { arrondir, format_size, getPercentage, storagePermission, timeFormat } from '../../services/helpersFunction';
...
const NewVideo = ({navigation, route}) => {
    ...
    var APIURL = baseUri + "saveVideo.php";
    ...
    const resetValues = () => {
        ...
        onHandleChangeText('description', '')
    }
    ...
    const fch = () => {
        ...
        var data = [
            ...
            { name: 'video', filename: inputs.video.name, type: inputs.video.type, data: RNFetchBlob.wrap(decodeURI(inputs.video.uri).replace('file://', '') )},//decodeURIComponent
        ...
        .then(resp => {
            ...
            if(response.success) {
                Alert.alert('Vidéo ajoutée avec succès.')
                resetValues()
            } else {
    ...
    const onHandlePress = () => {
        ...
        if(isValid) {
            console.log({video: inputs.video});
            setLoading(true)
            setPercentProgress(undefined)
            // fs();
            fch();
    ...
    const openPicker = async (mode) => {
        ...
        if(granted) {
            try {
                ...
                if(result) {
                    closePanel()
                    await hg(result, onChangeErrorText, onHandleChangeText, setCompressionActive, setCancellationVideoCompressId, setCompressorProgress, setDuration, setPoster, compress, false, clearPoster, setSizes)
                }
        ...
        <SafeAreaView style={{flex: 1, backgroundColor: '#000'}}>
            ...
            >
                <ScrollView contentContainerStyle={{paddingBottom: 50}}>
                    ...
                    <TextInputRN label='Description' tag='description' inputs={inputs} errors={errors} onHandleChangeText={onHandleChangeText} contentContainerStyle={styles.contentContainerInput} multiline={true} style={{ maxHeight: 250 }} />
